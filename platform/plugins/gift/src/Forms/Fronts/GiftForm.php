...
namespace Botble\Gift\Forms\Fronts;
...
class GiftForm extends FormFront
{
    public static function formTitle(): string
    {
        return trans('plugins/gift::gift.gift_form');
    }
    ...
    public function setup(): void
    {
        ...
        $this
            ->contentOnly()
            ->model(Gift::class)
            ->setUrl(route('public.send.gift'))
            ->setValidatorClass(GiftRequest::class)
            ->setFormOption('class', 'gift-form')
            ->add(
                'filters_before_form',
                HtmlField::class,
                HtmlFieldOption::make()
                    ->content(apply_filters('pre_gift_form', null))
                    ->toArray()
            )
            ->add(
                'required_fields',
                        ...
                        $form
                            ->add(
                                ...
                                TextFieldOption::make()
                                    ...
                                    ->label(__('Name'))
                                    ->placeholder(__('Name'))
                                    ->wrapperAttributes(['class' => 'gift-form-group'])
                                    ->cssClass('gift-form-input')
                                    ->maxLength(-1)
                                    ->toArray()
                            );
                    })
            ...
            ->add(
                ...
                HtmlField::class,
                HtmlFieldOption::make()
                    ->content(apply_filters('after_gift_form', null))
                    ->toArray()
            )
            ->addWrappedField(
                'submit',
            ...
            ->addWrappedField(
                ...
                HtmlFieldOption::make()
                    ->content(<<<'HTML'
                        <div class="gift-message gift-success-message" style="display: none"></div>
                        <div class="gift-message gift-error-message" style="display: none"></div>
                    HTML)
                    ->toArray()
            );
    }
    ...
    protected function addWrappedField(string $name, string $type, array $options): static
    {
        $this->add(
            ...
            HtmlField::class,
            HtmlFieldOption::make()->content('<div class="gift-form-group">')->toArray()
        );
    ...
    protected function addRowWrapper(string $name, Closure $callback): static
    {
        $this->add(
            ...
            HtmlField::class,
            HtmlFieldOption::make()->content('<div class="gift-form-row row">')->toArray()
        );
    ...
    protected function addColumnWrapper(string $name, Closure $callback, int $column = 6): static
    {
        $this->add(
            ...
            HtmlField::class,
            HtmlFieldOption::make()->content(sprintf('<div class="gift-column-%s col-md-%s gift-field-%s">', $column, $column, $name))->toArray()
        );
