...
$(document).on('click', '#conversation-type', async function () {
  ...
  try {
      ...
      if (keyToUpdate) {
          ...
          // Create new node
          await push(nodeRef, { chat_id: chat_id, conversation_type: newType });
      }
...
async function handleConversation(chat_id) {
  try {
    const nodeRef = ref(db, 'whatsapp_conversation_type');
    ...
    if (snapshot.exists()) {
      ...
      snapshot.forEach((childSnap) => {
        if (childSnap.val().chat_id === chat_id) 
        {
          ...
          if (childSnap.val().conversation_type === 'closed') {
      ...
      if (!exists) {
        ...
        $('#conversation-type').data({ chat_id }).toggleClass('btn-secondary btn-success').text('Close Conversation');
        console.log(`Chat ID ${chat_id} added with conversation_type 'open'.`);
      }
    } else {
      ...
      console.log(`Node created and Chat ID ${chat_id} added with conversation_type 'open'.`);
